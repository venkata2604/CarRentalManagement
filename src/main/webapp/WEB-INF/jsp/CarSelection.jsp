		...
		<div class="car-container">
			...
			<form action="CarSelectionServlet" method="post" class="form-style">
				...
				<div class="radio-group">
					<input type="radio" id="car<%=car.getCarId()%>"
						name="selectedCarId" value="<%=car.getCarId()%>"> <label
						for="car<%=car.getCarId()%>"><%=car.getMake()%> <%=car.getModel()%>
						- <%=car.getYear()%> | Seats: <%=car.getNumberOfSeats()%> | Price:
						$<%=car.getPricePerDay()%> | Status: <%=car.getStatus()%></label>
				</div>
				<%
				}
				%>
